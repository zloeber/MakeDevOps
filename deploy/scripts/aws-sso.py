...
parameter values (generally making it more docker friendly). Manually entered arguments will take precedence to the environmental variables defined below:
    ...
    fqdn - AWS_STS_SERVER -> ARG_FQDN = None
...
if __name__ == '__main__':
    ...
    ARG_FQDN = os.environ.get(
        "AWS_STS_SERVER", os.environ.get("ARG_FQDN", None))
    ...
    ARG_PROFILEPATH = os.environ.get("AWS_CONFIG_FILE", os.environ.get(
        "ARG_PROFILEPATH", os.path.join(os.path.expanduser("~"), '.aws', 'credentials')))
